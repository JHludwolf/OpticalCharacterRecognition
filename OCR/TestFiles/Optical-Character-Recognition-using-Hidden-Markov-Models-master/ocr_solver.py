...
class OCRSolver:
    ...
    def print_inputs(self):
        ...
        print('Size of emission probabilitie(', sys.getsizeof(self.emit_prob))
    ...
    def hmm_ve(self):
        ...
        for w in range(1, len(self.test_letters)):  # Calculating for the remaining characters
            ...
            chars[w] = char_dict
        #print('HMM VE:', ''.join( [chars[w].keys()[list(chars[w].values())[int(min(chars[w].values()))]] for w in range(len(self.test_letters))]))  # Printing the characters that have maximum probability
